...
class Controller:
    ...
    def run_frontend(self):
        ...
        services = ""
        services += frontend_template.format(path=self.root_path,
                                                gw="192.168.100.1",
                                                ip="192.168.100.100",
        ...
        docker_compose_file += network_template.format("192.168.100.1/24", "192.168.100.1")
        # Write the Docker Compose file in config directory
        with open(f"{os.path.join(os.environ['FEDSTELLAR_ROOT'], 'fedstellar', 'frontend', 'docker-compose.yml')}", "w") as f:
            f.write(docker_compose_file)

        # Start the Docker Compose file, catch error if any
        try:
            subprocess.check_call(["docker", "compose", "-f", f"{os.path.join(os.environ['FEDSTELLAR_ROOT'], 'fedstellar', 'frontend', 'docker-compose.yml')}", "up", "-d"])
